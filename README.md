...
**4. Выстави правильные права на файлы:**

```bash
# Сертификат — читается всеми
chmod 644 grafana/certs/cert.crt

# Приватный ключ — только владелец (UID 472 — пользователь grafana внутри контейнера)
chmod 600 grafana/certs/key.key
sudo chown 472:472 grafana/certs/key.key
```

> ⚠️ Без `chown 472:472` на `key.key` Grafana не сможет прочитать ключ и завершится с ошибкой (exit code 1) сразу при старте.

**5. Проверь, что сертификат и ключ совпадают:**
